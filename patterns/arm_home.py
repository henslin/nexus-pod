"""arm_home — registered LED-ring pattern (see pattern_common.py)."""
...
DURATION_MS = 9400
RENDER_ONLY = False
DESCRIPTION = ("Arm Home: amber CW comet x2 laps, solid 3 s, fade out.")


def schedule_arm_home(controller, system):
    """Arm Home: AMBER clockwise comet x2 laps -> solid amber -> fade out.
    Amber, never red, so Home stays distinct from Away at a glance. ~9.4 s."""
    return _schedule_spin_solid_fade(controller, system, ARM_HOME_AMBER)
